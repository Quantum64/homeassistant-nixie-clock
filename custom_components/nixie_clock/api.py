...
class NixieApiClient:
    ...
    async def _api_wrapper(
        ...
        if self._access_token is None:
            try:
                async with async_timeout.timeout(10):
                    ...
                    self._access_token = token_data["data"]["access_token"]
            except TimeoutError as exception:
                msg = f"Timeout error fetching information - {exception} {token_data}"
                raise NixieApiClientCommunicationError(
                    msg,
                ) from exception
            except (aiohttp.ClientError, socket.gaierror) as exception:
                msg = f"Error fetching information - {exception} {token_data}"
                raise NixieApiClientCommunicationError(
                    msg,
                ) from exception
            except Exception as exception:  # pylint: disable=broad-except
                msg = f"Something really wrong happened! - {exception} {token_data}"
        ...
        try:
            ...
            msg = f"Timeout error fetching information - {exception} req={str(data)} res={response.text()}"
            raise NixieApiClientCommunicationError(
                msg,
            ) from exception
        except (aiohttp.ClientError, socket.gaierror) as exception:
            msg = f"Error fetching information - {exception} req={str(data)} res={response.text()}"
            raise NixieApiClientCommunicationError(
                msg,
            ) from exception
        except Exception as exception:  # pylint: disable=broad-except
            msg = f"Something really wrong happened! - {exception} req={str(data)} res={response.text()}"
            raise NixieApiClientError(
                msg,
            ) from exception
